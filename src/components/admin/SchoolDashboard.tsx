...
export default function SchoolDashboard({
  ...
  const renderApprovalCard = (event: Event) => {
    return (
      <div
        key={event.id}
        className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-5 space-y-4 shadow-sm"
      >
        <div className="flex flex-col sm:flex-row justify-between gap-4 items-start">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-[#2A2621] uppercase">{event.title}</h4>
              <p className="text-[10px] text-[#5A554E] mt-0.5">
                Host: <span className="font-semibold text-[#5A554E]">{event.organizationName || event.organizer}</span>
              </p>
            </div>
          </div>
          ...
          <span className={`rounded-full px-2 py-0.5 text-[8px] font-extrabold uppercase tracking-wide border ${
            event.ownershipType === 'school'
              ...
              : 'bg-[#FD5C05]/15 border-[#FD5C05]/30 text-[#2A2621]'
          }`}>
            {event.ownershipType} event
          </span>
        </div>

        <p className="text-[11px] text-[#5A554E] bg-[#D8D2BC]/25 p-3 rounded-xl border border-black/[0.04] line-clamp-2">
          {event.description}
        </p>

        {/* Location & Time details */}
        <div className="grid gap-2 sm:grid-cols-3 text-[10px] text-[#5A554E] uppercase font-bold pt-1">
        ...
        {(event.fundingRequested || event.transportationNeeded) && (
          <div className="flex flex-wrap gap-2 text-[9px] bg-[#FD5C05]/10 p-2 rounded-lg border border-[#FD5C05]/20 text-[#2A2621] font-bold uppercase">
        ...
        <div className="flex flex-col sm:flex-row items-end gap-3 pt-3 border-t border-[#D8D2BC]/30">
          <div className="w-full space-y-1">
            <label className="text-[9px] font-bold text-[#5A554E] uppercase tracking-wide">Review Comment</label>
            <input
              type="text"
              ...
              className="w-full rounded-xl border border-black/10 bg-black/[0.01] px-3 py-2 text-xs text-[#2A2621] placeholder-[#5A554E] focus:outline-none focus:border-[#FD5C05]"
            />
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              disabled={processing !== null}
              className="flex items-center gap-1 rounded-xl bg-[#FD5C05] hover:bg-[#CC3D00] text-[#2A2621] border border-[#FD5C05]/30 px-4 py-2 text-[10px] font-bold uppercase transition-colors cursor-pointer"
            >
              <CheckCircle className="h-4 w-4 text-[#2A2621]" /> Approve
            </button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[28px] border border-[#D8D2BC]/30 bg-[#D8D2BC]/35 p-6 flex items-center justify-between shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-xs text-[#5A554E] max-w-xl leading-relaxed">
      ...
      {/* Sub-tab navigation */}
      <div className="flex space-x-1 rounded-xl bg-[#D8D2BC]/40 p-1 border border-black/[0.04] w-fit text-xs font-bold text-[#5A554E]">
        <button
          onClick={() => setSubTab('overview')}
          className={`rounded-lg px-4 py-2 transition-all cursor-pointer ${
            subTab === 'overview' ? 'bg-[#FD5C05] text-[#2A2621] shadow-sm' : 'hover:text-[#2A2621]'
          }`}
        >
          Overview KPIs
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 transition-all cursor-pointer ${
            subTab === 'approvals' ? 'bg-[#FD5C05] text-[#2A2621] shadow-sm' : 'hover:text-[#2A2621]'
          }`}
        >
        ...
        <button
          ...
          className={`rounded-lg px-4 py-2 transition-all cursor-pointer ${
            subTab === 'orgs' ? 'bg-[#FD5C05] text-[#2A2621] shadow-sm' : 'hover:text-[#2A2621]'
          }`}
        >
          Organizations List
        </button>
        <button
          ...
          className={`rounded-lg px-4 py-2 transition-all cursor-pointer ${
            subTab === 'featured' ? 'bg-[#FD5C05] text-[#2A2621] shadow-sm' : 'hover:text-[#2A2621]'
          }`}
        >
          Featured Manager
        </button>
      ...
      <div className="space-y-6">
        ...
        {subTab === 'overview' && (
          ...
          <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-5 space-y-1 shadow-sm">
                <span className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Total Postings</span>
                <p className="text-2xl font-bold text-[#2A2621]">{totalPostings}</p>
              ...
              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-5 space-y-1 shadow-sm">
                <span className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Pending Reviews</span>
                <p className="text-2xl font-bold text-[#2A2621]">{pendingEvents.length}</p>
                <p className="text-[10px] text-[#5A554E]">Needs administrative actions</p>
              </div>

              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-5 space-y-1 shadow-sm">
                <span className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Total RSVPs</span>
                <p className="text-2xl font-bold text-[#2A2621]">{totalRSVPs}</p>
                <p className="text-[10px] text-[#5A554E]">Across all experiences</p>
              </div>

              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-5 space-y-1 shadow-sm">
                <span className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Groups Registered</span>
                <p className="text-2xl font-bold text-[#2A2621]">{organizations.length}</p>
                <p className="text-[10px] text-[#5A554E]">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {/* Popular categories */}
              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-6 space-y-4 shadow-sm">
                <h3 className="text-xs font-bold text-[#5A554E] uppercase tracking-widest flex items-center gap-1.5">
                  <Layers className="h-4 w-4 text-[#2A2621]" />
                  Event Queues Split
                </h3>
                
                <div className="space-y-3 text-xs">
                  <div className="space-y-1.5">
                    <div className="flex justify-between font-bold text-[#5A554E]">
                      <span>Fast Track Queue (Quick)</span>
                      <span>{events.filter(e => e.complexityType === 'quick').length}</span>
                    </div>
                    <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden border border-[#D8D2BC]/40">
                      <div
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="h-2 w-full rounded-full bg-slate-100 overflow-hidden border border-[#D8D2BC]/40">
                      <div
                        className="h-full bg-[#FD5C05] rounded-full"
              ...
              <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-6 space-y-4 shadow-sm">
                <h3 className="text-xs font-bold text-[#5A554E] uppercase tracking-widest flex items-center gap-1.5">
                  <BarChart3 className="h-4 w-4 text-[#2A2621]" />
                ...
                <div className="space-y-3 text-xs">
                  {organizations.slice(0, 3).map((org) => (
                    <div key={org.id} className="flex items-center justify-between gap-4">
                      <span className="font-bold text-[#5A554E] w-1/2 truncate uppercase">{org.name}</span>
                      <div className="w-1/2 flex items-center gap-3">
                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden border border-[#D8D2BC]/40">
                          <div
                            className="h-full bg-gradient-to-r from-[#FD5C05] to-[#D8D2BC] rounded-full"
                            style={{ width: `${(org.rsvps || 0) / 2}%` }}
                          />
                        </div>
                        <span className="font-bold text-[#2A2621] shrink-0">{org.rsvps} RSVPs</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {subTab === 'approvals' && (
          ...
          <div className="space-y-6">
            ...
            {/* Queue switch */}
            <div className="flex space-x-2 border-b border-[#D8D2BC]/30 pb-3 text-xs font-bold text-[#5A554E]">
              <button
                onClick={() => setActiveQueue('fast')}
                className={`pb-3 border-b-2 transition-colors cursor-pointer ${
                  activeQueue === 'fast' ? 'border-[#FD5C05] text-[#2A2621]' : 'border-transparent hover:text-[#2A2621]'
                }`}
              >
              ...
              <button
                ...
                className={`pb-3 border-b-2 transition-colors cursor-pointer ${
                  activeQueue === 'standard' ? 'border-[#FD5C05] text-[#2A2621]' : 'border-transparent hover:text-[#2A2621]'
                }`}
              >
              ...
              <button
                ...
                className={`pb-3 border-b-2 transition-colors cursor-pointer ${
                  activeQueue === 'complex' ? 'border-[#FD5C05] text-[#2A2621]' : 'border-transparent hover:text-[#2A2621]'
                }`}
              >
            ...
            <div className="space-y-4">
              {activeQueue === 'fast' && (
                fastQueue.length === 0 ? (
                  <p className="text-xs text-[#5A554E] italic text-center py-10">Fast track queue cleared!</p>
                ) : (
              ...
              {activeQueue === 'standard' && (
                standardQueue.length === 0 ? (
                  <p className="text-xs text-[#5A554E] italic text-center py-10">Standard queue cleared!</p>
                ) : (
              ...
              {activeQueue === 'complex' && (
                complexQueue.length === 0 ? (
                  <p className="text-xs text-[#5A554E] italic text-center py-10">Complex queue cleared!</p>
                ) : (
      ...
      <div key={org.id} className="rounded-xl border border-[#D8D2BC]/30 bg-white p-4 shadow-sm">
        ...
        {subTab === 'orgs' && (
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            {/* Registered list */}
            <div className="md:col-span-2 rounded-[24px] border border-[#D8D2BC]/30 bg-white p-6 space-y-4 shadow-sm">
              <h4 className="text-xs font-bold text-[#5A554E] uppercase tracking-widest">Registered Groups ({organizations.length})</h4>
              <div className="space-y-3">
                {organizations.map((org) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-9 w-9 rounded-xl bg-[#FD5C05] flex items-center justify-center text-[#2A2621] font-extrabold text-xs shadow-sm">
                        {org.name.slice(0,2).toUpperCase()}
                      </div>
                      <div>
                        <h5 className="text-xs font-bold text-[#2A2621] uppercase">{org.name}</h5>
                        <p className="text-[10px] text-[#5A554E]">Registered by student members</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={`rounded-full px-4 py-1.5 text-[9px] font-bold uppercase tracking-wider border transition-colors cursor-pointer ${
                        org.verified
                          ...
                          : 'bg-[#FD5C05] text-[#2A2621] border-[#D8D2BC]/40 hover:bg-[#CC3D00]'
                      }`}
                    >
            ...
            {/* Create organization */}
            <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-6 space-y-4 shadow-sm h-fit">
              <div className="space-y-1.5">
                <h4 className="text-xs font-bold text-[#2A2621] uppercase tracking-widest flex items-center gap-1.5">
                  <Building className="h-4 w-4 text-[#2A2621]" />
                  Create Club Profile
                </h4>
                <p className="text-[10px] text-[#5A554E]">Establish verified campus group files instantly.</p>
              </div>

              <form onSubmit={handleOrgSubmit} className="space-y-4 pt-2">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[#5A554E] uppercase tracking-wide">Club Title</label>
                  <input
                    type="text"
                    required
                    value={orgName}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-xl border border-black/10 bg-black/[0.01] px-3 py-2 text-xs text-[#2A2621] placeholder-[#5A554E] focus:outline-none focus:border-[#FD5C05] resize-none"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[#5A554E] uppercase tracking-wide">Color Scheme</label>
                  <select
                    value={orgColor}
                    onChange={(e) => setOrgColor(e.target.value)}
                    className="w-full rounded-xl border border-black/10 bg-white px-3 py-2 text-xs text-[#2A2621] focus:outline-none focus:border-[#FD5C05]"
                  >
                ...
                <button
                  type="submit"
                  className="w-full rounded-full bg-[#FD5C05] py-3 text-xs font-bold text-[#2A2621] shadow-md shadow-[#FD5C05]/25 hover:bg-[#CC3D00] transition-colors cursor-pointer"
                >
                  Register Club
                </button>
              </form>
        ...
        {subTab === 'featured' && (
          ...
          <div className="rounded-[24px] border border-[#D8D2BC]/30 bg-white p-6 space-y-4 shadow-sm">
            <h4 className="text-xs font-bold text-[#5A554E] uppercase tracking-widest">Featured Events Selector</h4>
            <div className="space-y-3 text-xs">
              {approvedEvents.map((event) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h5 className="font-extrabold text-[#2A2621] uppercase">{event.title}</h5>
                      <span className="text-[10px] text-[#5A554E] uppercase">
                  ...
                  <button
                    ...
                    className={`rounded-full px-4 py-1.5 text-[9px] font-bold uppercase tracking-wider border transition-colors cursor-pointer ${
                      event.featured
                        ? 'bg-[#FD5C05]/25 text-[#2A2621] border-[#FD5C05]/30'
                        : 'bg-white border-black/10 text-[#5A554E] hover:bg-[#FD5C05]'
                    }`}
                  >
