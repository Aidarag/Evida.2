...
export default function EventDetails({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-[#191919]/60 backdrop-blur-md" onClick={onClose}></div>

      {/* Modal invitation */}
      <div className="relative w-full max-w-2xl overflow-hidden rounded-[32px] border border-black/[0.06] bg-white text-[#191919] shadow-2xl transition-all max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative aspect-[16/9] w-full overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute right-6 top-6 z-20 h-10 w-10 rounded-full bg-white/90 hover:bg-white border border-black/10 flex items-center justify-center text-[#191919] transition-colors cursor-pointer shadow-sm"
          >
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="absolute inset-x-6 bottom-6 z-20 space-y-1">
            <span className="rounded-full bg-[#BDFB04] text-[#191919] text-[9px] font-bold uppercase tracking-wider px-2.5 py-0.5 w-fit border border-[#BDFB04]/30 shadow-sm">
              {event.free ? 'FREE ACCESS' : 'TICKETED'}
            </span>
            <h2 className="text-xl sm:text-2xl font-extrabold text-[#191919] uppercase tracking-tight leading-none mt-2 line-clamp-3 bg-white/95 px-3 py-1.5 rounded-2xl w-fit shadow-sm border border-black/5">
              {event.title}
            </h2>
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="rounded-2xl border-l-4 border-[#BDFB04] bg-[#BDFB04]/10 p-4 flex gap-3 text-xs leading-relaxed border border-transparent shadow-sm">
          ...
          <div className="flex items-center justify-between border-t border-black/[0.06] pt-4 text-xs">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-lg bg-[#DFDED7]/50 hover:bg-black/[0.04] border border-black/5 text-[#191919] font-bold px-3 py-1.5 text-[10px] uppercase transition-all cursor-pointer shadow-sm"
              >
              ...
              <button
                ...
                className="rounded-lg bg-[#DFDED7]/50 hover:bg-black/[0.04] border border-black/5 text-[#191919] font-bold px-3 py-1.5 text-[10px] uppercase transition-all cursor-pointer flex items-center gap-1 shadow-sm"
              >
                <Share2 className="h-3 w-3 text-[#191919]" />
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 rounded-full px-5 py-3 text-xs font-bold transition-all hover:scale-[1.01] cursor-pointer ${
                isSaved
                  ...
                  : 'bg-white border border-black/10 text-[#191919] hover:bg-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 rounded-full py-3.5 text-xs font-bold transition-all hover:scale-[1.01] cursor-pointer shadow-lg ${
                isRsvped
                  ...
                  : 'bg-[#BDFB04] text-[#191919] shadow-[#BDFB04]/25 hover:bg-[#d1fa3c]'
              }`}
            >
