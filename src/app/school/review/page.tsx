      ...
      <div className="space-y-4">
        <AnimatePresence mode="popLayout">
          {displayEvents.length > 0 ? displayEvents.map(event => (
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-[#B8BBC8] mt-1 line-clamp-2">{event.description}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs font-medium text-[#B8BBC8]">
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-[#80B0EC]" />
                      {new Date(event.date).toLocaleDateString()} at {event.time}
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="h-4 w-4 text-[#80B0EC]" />
                      {event.location}
                    </div>
                    <div className="flex items-center gap-2 col-span-2 border-t border-white/[0.06] pt-3 mt-1">
                      <Users className="h-4 w-4 text-white" />
                      Hosted by: <span className="text-white">{event.organizationName || event.organizer}</span>
                    </div>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex md:flex-col justify-end gap-3 md:w-32 md:border-l md:border-white/[0.06] md:pl-6">
                  <Button 
                    variant="neon" 
                    size="sm" 
                    fullWidth 
                  ...
                  <Button 
                    ...
                    size="sm" 
                    fullWidth 
                    className="hover:bg-[#EE3D5A]/10 hover:text-[#EE3D5A] hover:border-[#EE3D5A]/30"
            ...
            <EmptyState
              icon={<ClipboardList className="h-8 w-8 text-[#B8BBC8]" />}
